...
export default function Profile() {
    ...
    const copylink = async (id) => {
        navigator.clipboard.writeText(`https://framefeet.vercel.app/api/share/${id}`)
    }
        ...
        <div className="w-screen h-screen bg-white mt-[80px] pt-4 flex justify-start items-center flex-col">
            {
                result?.data?.length != 0 ? result?.data?.map((item, index) => (
                    <Accordion className="border-2 border-slate-900 w-[50%] mt-4 rounded-lg" key={index}>
                        <AccordionSummary
                        ...
                        <AccordionDetails className="flex justify-center items-center flex-col w-full">
                            ...
                            <div className="w-full flex justify-between items-center flex-row my-2">
                                ...
                                <button onClick={() => copylink(item.payment_id)} className="bg-lime-400 p-2 text-sm text-black font-semibold rounded-full px-4 flex justify-between items-center border-2 border-purple-500">
